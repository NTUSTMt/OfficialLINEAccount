...
## 📌 版本資訊 (Version Info)
- **當前版本**：`0.1.123` (v0.1.123)

---

## 🛠️ 主要更新與修復 (Key Updates & Bug Fixes)

### 223. 發布社員全方位使用指南 (MEMBER_GUIDE.md) 與 LINE Bot 更多服務深度整合 (v0.1.123)
- **需求背景與設計目標 (Background & Objectives)**：
  - 為使所有新舊社員與國際山友能無障礙使用台科登山社線上系統，撰寫一份架構清晰、好讀且完整的**「社員全方位使用指南」([MEMBER_GUIDE.md](file:///Users/brianhung/Documents/OfficialLINEAccount/MEMBER_GUIDE.md))**。
  - 依照使用者需求，**採取繁體中文版與英文版完全獨立分開的兩大部分 (Part I & Part II)**，避免中英句間混雜帶來的閱讀疲勞，提供最佳雙語體驗。
- **架構設計與落實細節 (Architecture & Implementation)**：
  1. **獨立繁中／英文專屬指南手冊 ([MEMBER_GUIDE.md](file:///Users/brianhung/Documents/OfficialLINEAccount/MEMBER_GUIDE.md))**：
     - **系統視覺化架構圖**：提供完整 Mermaid 流程圖，梳理 LINE 官方帳號客戶端、LIFF 頂部通用導航列與各功能頁面的交互關係。
     - **四大頁面切換途徑教學**：詳細解說「底部圖文選單 (Rich Menu)」、「網頁頂部頭像選單 (Avatar Dropdown)」、「頁面內深層捷徑 (Deep Links)」與「聊天文字指令 (Chat Keywords)」四種跳轉方式。
     - **六大登山業務全流程引導**：
       - 📝 **個人資料**：深入說明 6 大必填欄位對活動入山證、留守安全與保險的重要意義。
       - 🏕️ **最新活動**：介紹路線難度、防重複報名機制與五大審核狀態轉換（審核中 ➔ 正取/備取 ➔ 正取（已繳費））。
       - 🎒 **裝備租借**：庫存查詢、社員專屬「租金 5 折優惠」自動折抵、押金與借還點交規範。
       - 💳 **繳費申報**：多筆項目合併申報機制、末五碼填寫防呆、最新標準狀態 `已核銷 Confirmed` 與歷史花費統計。
       - 📊 **個人主頁**：掌握個人名下活動、裝備與待繳費用，並建立自請取消與候補自動遞補指引。
       - 🏆 **心得與成就**：出隊心得評分回饋、百岳與中級山足跡徽章解鎖。
       - 🤖 **小岳助理**：1 對 1 私聊提問、群組 `@小岳助理` 召喚技巧與指令索引。
       - ❓ **常見問答 FAQ**：收錄 iOS WebKit 瀏覽器暫存排除、個資阻擋解除、核銷對帳時程與代出隊禁令。
  2. **LINE Bot 更多服務卡片擴充 ([gas_modules/03_Flex_Templates.js](file:///Users/brianhung/Documents/OfficialLINEAccount/gas_modules/03_Flex_Templates.js), [src/gas.js](file:///Users/brianhung/Documents/OfficialLINEAccount/src/gas.js))**：
     - 在「🛠️ 更多服務 (More Services)」選單中新增「📖 社員使用指南 Member Guide」按鈕，點擊後自動傳送 `使用指南`。
  3. **LINE Bot Webhook 智慧關鍵字應答 ([gas_modules/02_LineBot_Webhook.js](file:///Users/brianhung/Documents/OfficialLINEAccount/gas_modules/02_LineBot_Webhook.js), [src/gas.js](file:///Users/brianhung/Documents/OfficialLINEAccount/src/gas.js))**：
     - 支援社員在聊天室中輸入「使用指南」、「操作指南」、「用戶手冊」、「社員指南」、「member guide」、「user guide」等關鍵字，自動回傳雙語精華導覽卡片與手冊查閱引導。
- **測試與品質驗證 (Verification)**：
  - 單元測試：新增第 58 組測試，`pnpm test` **149/149 全數通過（40 test suites, 0 failures）**。
  - 前端打包：`pnpm run build` TypeScript 編譯與 Vite 建置成功完成，無任何警告或錯誤。
